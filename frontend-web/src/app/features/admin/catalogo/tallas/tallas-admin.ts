...
import { Talla } from '../../../../core/models/catalogo.model';

@Component({
  selector: 'app-tallas-admin',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="page-header">
      <h1>Tallas</h1>
      <p>Lista maestra de tallas disponibles.</p>
    </div>
    ...
    <div class="card" style="margin-bottom: 1.5rem;">
      <form (ngSubmit)="crear()" class="form-row-inline">
        <div class="form-group">
          <label>Nombre</label>
          <input type="text" [(ngModel)]="nombre" name="nombre" placeholder="Ej. M" required />
        </div>
...
export class TallasAdmin implements OnInit {
  ...
  cargar() {
    this.service.listarTallas().subscribe((data) => this.items.set(data));
  }

  crear() {
    this.guardando.set(true);
    this.service.crearTalla(this.nombre).subscribe({
      next: () => {
        this.guardando.set(false);
        this.nombre = '';
        this.mostrarToast('Talla creada');
        this.cargar();
      },
  ...
  eliminar(t: Talla) {
    this.service.eliminarTalla(t.id).subscribe(() => {
